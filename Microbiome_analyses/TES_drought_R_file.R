...
species = species[species$length %in% c("timezero","150d"),]




NAMES = rownames(species)
g_matrix = species[,9:1374]   #1287
rownames(g_matrix) = NAMES
g_matrix = as.matrix(g_matrix)
...
ggplot(data=pcoa_plot_merged,aes(x=Axis.1,y=-Axis.2)) + 
  geom_point(aes(fill=factor(length),shape=Site), colour="black", size=6,alpha=0.6) + theme_bw()  +
...
ggplot(data=pcoa_plot_merged,aes(x=Axis.1,y=Axis.2,group=Site)) + geom_point(aes(fill=factor(Site)),shape=21, colour="black", size=6) + theme_bw()  +
  theme_bw(base_size=14) + 
...
adonis(g_matrix~g_sample$Site,method="bray",permutations=999)

ggplot(data=pcoa_plot_merged,aes(x=Axis.1,y=Axis.2,group=length)) + geom_point(aes(fill=factor(length)),shape=21, colour="black", size=6) + theme_bw()  +
  theme_bw(base_size=14) + 
  stat_ellipse(aes(color=length),type="norm",level=0.95)+
  theme(axis.text=element_text(size=14,color="black"),axis.title=element_text(size=14),legend.background = element_rect(colour = "black"),
        legend.text = element_text(size=18), legend.title=element_text(size=20)) + labs(fill = "Group")+theme(legend.title=element_blank())+
  labs(x = paste("PC1 - Variation Explained", round(PC1,2),"%"), y = paste("PC2 - Variation Explained", round(PC2,2),"%"))

adonis(g_matrix~g_sample$length,method="bray",permutations=999)
pairwise.adonis(bray_distance,factors=g_sample$length)
betadisp=betadisper(bray_distance,g_sample$length,type="centroid")

anova(betadisp)
TukeyHSD(betadisp)

ggplot(data=pcoa_plot_merged,aes(x=Axis.1,y=Axis.2,group=location)) + geom_point(aes(fill=factor(location)),shape=21, colour="black", size=6) + theme_bw()  +
  theme_bw(base_size=14) + 
  stat_ellipse(aes(color=location),type="norm")+
  theme(axis.text=element_text(size=14,color="black"),axis.title=element_text(size=14),legend.background = element_rect(colour = "black"),
        legend.text = element_text(size=18), legend.title=element_text(size=20)) + labs(fill = "Group")+theme(legend.title=element_blank())+
  labs(x = paste("PC1 - Variation Explained", round(PC1,2),"%"), y = paste("PC2 - Variation Explained", round(PC2,2),"%"))
...
#species = read.table("taxtable7_transposed.txt", sep="\t", header=TRUE,row.names=1)

ggplot(data=pcoa_plot_merged,aes(x=Axis.1,y=Axis.2,group=drying)) + geom_point(aes(fill=factor(drying)),shape=21, colour="black", size=6) + theme_bw()  +
  theme_bw(base_size=14) + 
  stat_ellipse(aes(color=drying),type="norm")+
  theme(axis.text=element_text(size=14,color="black"),axis.title=element_text(size=14),legend.background = element_rect(colour = "black"),
        legend.text = element_text(size=18), legend.title=element_text(size=20)) + labs(fill = "Group")+theme(legend.title=element_blank())+
  labs(x = paste("PC1 - Variation Explained", round(PC1,2),"%"), y = paste("PC2 - Variation Explained", round(PC2,2),"%"))

adonis(g_matrix~g_sample$drying,method="bray",permutations=999)


ggplot(data=pcoa_plot_merged,aes(x=Axis.1,y=Axis.2,group=saturation)) + geom_point(aes(fill=factor(saturation)),shape=21, colour="black", size=6) + theme_bw()  +
  theme_bw(base_size=14) + 
...
adonis(g_matrix~g_sample$saturation,method="bray",permutations=999)

ggplot(data=pcoa_plot_merged,aes(x=Axis.1,y=Axis.2,group=rep)) + geom_point(aes(fill=factor(rep)),shape=21, colour="black", size=6) + theme_bw()  +
  theme_bw(base_size=14) + 
  stat_ellipse(aes(color=rep))+
  theme(axis.text=element_text(size=14,color="black"),axis.title=element_text(size=14),legend.background = element_rect(colour = "black"),
        legend.text = element_text(size=18), legend.title=element_text(size=20)) + labs(fill = "Group")+theme(legend.title=element_blank())+
  labs(x = paste("PC1 - Variation Explained", round(PC1,2),"%"), y = paste("PC2 - Variation Explained", round(PC2,2),"%"))

adonis(g_matrix~g_sample$rep,method="bray",permutations=999)
...
#species = species[species$saturation %in% c("saturated","timezero"), ]
#species = species[species$saturation %in% "saturated", ]
species = species[species$drying %in% c("FAD","CW", NA),]
species = species[species$length %in% c("150d","timezero"),]
species = species[species$depth %in% c("0-5cm"),]
...
shannon_merged = merge(shannon_merged,richness,by="Row.names")
shannon_merged$richness = richness
shannon_merged$group = c("c","c","c","f","f","f","c","c","c","f","f","Z","Z","Z")


ggplot(shannon_merged, aes(x=drying,y=shannon))+
  geom_boxplot()+
  theme_bw()
  
species_anova = aov(shannon~group,data=shannon_merged)
summary(species_anova)
TukeyHSD(species_anova)


 t.test(shannon~drying, data=shannon_merged)

ggplot(shannon_merged, aes(x=drying,y=richness))+
  geom_boxplot()+
  theme_bw()

species_anova = aov(richness~group,data=shannon_merged)
summary(species_anova)
TukeyHSD(species_anova)


t.test(richness~length,data=shannon_merged)
...
t.test(shannon_diversity~Site,data=shannon_merged)
...
ggplot(shannon_merged, aes(x=length,y=shannon_diversity))+
  geom_boxplot()+
  facet_wrap(~Site)+
  theme_bw()
...
species_anova = aov(shannon_diversity~length,data=shannon_merged)
summary(species_anova)
TukeyHSD(species_anova)

shannon_NA_location = na.omit(shannon_merged,cols=location)

ggplot(shannon_NA_location, aes(x=location,y=shannon_diversity))+
  geom_boxplot()+
  facet_wrap(~Site)+
  theme_bw()

t.test(shannon_diversity~location,data=shannon_NA_location)


shannon_NA_drying = na.omit(shannon_merged,cols=drying)
ggplot(shannon_NA_drying, aes(x=drying,y=shannon_diversity))+
  facet_wrap(~Site)+
  geom_boxplot()+
  theme_bw()

#
t.test(shannon_diversity~drying,data=shannon_NA_drying)



ggplot(shannon_merged, aes(x=saturation,y=shannon_diversity))+
  geom_boxplot()+
  facet_wrap(~Site)+
  theme_bw()

#
t.test(shannon_diversity~saturation,data=shannon_merged)
species_anova = aov(shannon_diversity~saturation,data=shannon_merged)
summary(species_anova)
TukeyHSD(species_anova)
...
richness_merged = merge(g_sample, richness, by="row.names")

#
ggplot(richness_merged,aes(x=depth,y=richness))+
  geom_boxplot()+
  facet_wrap(~drying)+
  theme_bw()
t.test(richness~depth,data=richness_merged)


ggplot(richness_merged,aes(x=Site,y=richness))+
  geom_boxplot()+
  theme_bw()
t.test(richness~Site,data=richness_merged)

richness_merged$length = as.character(richness_merged$length)
richness_merged$length = factor(richness_merged$length, levels=c("timezero","30d","90d","150d"))

#facet_wrap(~drying)+
ggplot(richness_merged,aes(x=length,y=richness))+
  geom_boxplot()+
  
  theme_bw()
species_anova = aov(richness~length,data=richness_merged)
summary(species_anova)
TukeyHSD(species_anova)

richness_NA_location = na.omit(richness_merged,cols=location)

ggplot(richness_NA_location,aes(x=location,y=richness))+
  geom_boxplot()+
  facet_wrap(~Site)+
  theme_bw()
t.test(richness~location,data=richness_NA_location)

#
richness_NA_drying = na.omit(richness_merged,cols=drying)
ggplot(richness_NA_drying,aes(x=drying,y=richness))+
  geom_boxplot()+
  facet_wrap(~Site)+
  theme_bw()
t.test(richness~drying,data=richness_NA_drying)

#
ggplot(richness_merged,aes(x=saturation,y=richness))+
  geom_boxplot()+
  facet_wrap(~Site)+
  theme_bw()
t.test(richness~saturation,data=richness_merged)
species_anova = aov(richness~saturation,data=richness_merged)
summary(species_anova)
TukeyHSD(species_anova)
...
combined_fixed %>%
  ...
  facet_wrap(~ depth+Site,scales="free")+
  labs(x=NULL,y= NULL)+
  theme(
